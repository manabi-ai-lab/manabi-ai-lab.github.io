...
2026-08-06 時点で、Drive のバックアップフォルダは
**「リンクを知っている全員」に `writer`（編集者）権限**が付いています。

リンクが漏れると、**第三者がパーツを上書き・削除できる状態**です。
復元と Release への添付が済んだら、**`reader`（閲覧者）に落とす**ことを強くおすすめします。

Drive → フォルダを右クリック → 共有 → 「リンクを知っている全員」の役割を **閲覧者** に変更。
